process GTDBTK_CLASSIFYWF {
    ...
    input:
tuple val(meta), path("bins/*", stageAs: { file -> 
    def name = file.name
    // Replace assembly/scaffolds/contigs/unitigs patterns with standardized name
    name.replaceAll(/\.(assembly|scaffolds|contigs|unitigs)\.(fa|fasta|fna|fas|fsa)\.gz$/, '.fa.gz')
})
    ...
    tuple val(meta), path("${prefix}/classify/*.gtdbtk_taxonomy.tsv"), emit: summary
    ...
    awk 'FNR==1 && NR!=1{next} {print}' \
        ${prefix}/classify/${prefix}.*.summary.tsv \
        2>/dev/null > ${prefix}.gtdbtk_taxonomy.tsv || touch ${prefix}.gtdbtk_taxonomy.tsv
    ...
    touch ${prefix}.gtdbtk_taxonomy.tsv
