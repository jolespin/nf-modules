...
include { GTDBTK_CLASSIFYWF_WITH_STAGING } from "../main"

workflow {
    // Collect all genome files into single channel with meta
    all_genomes_ch = Channel.fromPath(params.genome_1, checkIfExists:true)
        .mix(Channel.fromPath(params.genome_2, checkIfExists:true))
    	.mix(Channel.fromPath(params.genome_3, checkIfExists:true))
        .collect()
        .map { files ->
    ...
    // Run the process
    GTDBTK_CLASSIFYWF_WITH_STAGING(
        all_genomes_ch,
        db_ch,
    ...
    // View output
    GTDBTK_CLASSIFYWF_WITH_STAGING.out.summary.view()
}
