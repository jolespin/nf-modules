...
process STROBEALIGN_WRAPPER {
    ...
    conda "bioconda::strobealign=0.17.0 jolespin::fastq_preprocess=2026.4.21"
    ...
    tuple val(meta), path('*.coverage.tsv.gz')       , optional: true, emit: coverage
    tuple val(meta), path('*.breadth.tsv.gz')       , optional: true, emit: breadth
    ...
    def breadth_command = save_bam ? "coverage_breadth --depth ${prefix}.mapped.sorted.bam.depth.tsv.gz -o ${prefix}.mapped.sorted.bam.breadth.tsv.gz ${contigs_arg}" : ""
    """
    # strobealign
    strobealign_wrapper \\
        ${bam_args} \\
    ...
    stub:
    def prefix = task.ext.prefix ?: "${meta.id}"
    def stub_bam = save_bam ? "touch ${prefix}.mapped.sorted.bam ${prefix}.mapped.sorted.bam.bai ${prefix}.mapped.sorted.bam.depth.tsv.gz ${prefix}.mapped.sorted.bam.coverage.tsv.gz ${prefix}.mapped.sorted.bam.breadth.tsv.gz" : ""
